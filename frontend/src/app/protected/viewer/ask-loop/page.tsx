...
export default function ViewerAskLoop() {
  ...
  async function ask(value: string) {
    ...
    try { const response = await apiClient.post('/ask-loop/ask', { question: value }); setAnswer(response.data.data.answer); }
    catch (error) { setAnswer(getErrorMessage(error)); }
    ...
    <div className={styles.content}>
      ...
      <section className={styles.layout}>
        ...
        <section className={styles.chat}>
          <div className={styles.question}>{question || 'What are the top issues customers are facing this week?'}<small>May 17, 2024 10:24 AM</small></div>
