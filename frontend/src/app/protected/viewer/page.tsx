...
function EmptyState({ label }: { label: string }) {
  return <div className={styles.emptyState}><span>✦</span><strong>{label}</strong><small>Your feedback data will appear here when it is available.</small></div>;
}
...
export default function ViewerPage() {
  ...
  return <main className={styles.page}>
    <aside className={styles.sidebar}>
      ...
      <div className={styles.sideBottom}><small>WORKSPACE</small><button><Network size={18} /> Acme Corp <ChevronDown size={15} /></button><button className={styles.viewer} onClick={() => router.push('/protected/viewer/profile')} aria-label="Open profile"><i>{initials}</i><span>You are viewing as<strong>Viewer</strong></span><em /></button><button className={styles.help}><CircleHelp size={19} /> Need Help?</button><button onClick={() => { logout(); router.replace('/auth/login'); }}><LogOut size={19} /> Logout</button></div>
    </aside>
    <div className={styles.content}>
      <header className={styles.top}><Menu size={28} /><div><h1>Viewer Dashboard <BarChart3 size={19} /></h1><p>Read-only access to feedback insights and reports.</p></div><button className={styles.date}>{dateLabel}</button><CircleHelp size={25} /><button className={styles.role} onClick={() => router.push('/protected/viewer/profile')} aria-label="Open profile"><i>{initials}</i><span>Viewer<small>Read-only access</small></span><ChevronDown size={15} /></button></header>
      <div className={styles.grid}>
        ...
        <section className={styles.four}><Card title="Top Themes">{(data?.topThemes ?? []).slice(0,6).map((t,i) => <div className={styles.theme} key={t.id}><b>{i+1}</b><span>{t.name}</span><strong>{t.count.toLocaleString()}</strong><em>{t.percentage}%</em></div>)}{!data?.topThemes.length && <EmptyState label="No themes discovered" />}</Card><Card title="Trends & Spikes">{trend.length ? trend.slice(-4).map((p,i) => <div className={styles.spike} key={p.date}><i>↗</i><span>{p.date}<small>{p.count.toLocaleString()} feedback</small></span><strong>Live</strong></div>) : <EmptyState label="No spikes detected" />}</Card><Card title="AI Insights (Top)">{(data?.insights ?? []).slice(0,3).map((insight) => <div className={styles.insight} key={insight.title}>{insight.title}<small>{insight.description}</small></div>)}{!data?.insights.length && <EmptyState label="Insights will appear here" />}</Card><Card title="Recent Activity">{(feedback.data?.items ?? []).slice(0,4).map(item => <div className={styles.activity} key={item.id}><i>◈</i><span>Feedback received<small>{item.source} · {format(new Date(item.createdAt), 'MMM d')}</small></span></div>)}{!feedback.data?.items.length && <EmptyState label="No recent activity" />}</Card></section>
