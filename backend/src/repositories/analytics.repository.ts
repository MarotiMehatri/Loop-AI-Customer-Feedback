...
function dateWhere(
  ...
  const w: Prisma.FeedbackWhereInput = { workspaceId };
  if (start || end) {
    w.createdAt = {};
...
export async function getShipmentTrend(
  workspaceId: string,
  start?: Date,
  end?: Date,
  groupBy?: string,
...
export async function getCourierPerformanceComparison(
  workspaceId: string,
  start?: Date,
  end?: Date,
  courierId?: string,
...
export async function getCourierDeliveryMetrics(
  workspaceId: string,
  start?: Date,
  end?: Date,
  courierId?: string,
...
export async function getCourierTrendData(
  workspaceId: string,
  start?: Date,
  end?: Date,
  courierId?: string,
  ...
  return Object.entries(grouped).map(([date, couriers]) => ({
    date,
    couriers,
  }));
}
...
export async function getCustomerSatisfactionScores(
  workspaceId: string,
  start?: Date,
  end?: Date,
  customerId?: string,
  ...
  return Object.entries(scores).map(([customerId, score]) => ({
    customerId,
    ...score,
  }));
}
...
export async function getCustomerOrderFrequency(
  ...
  shipments.forEach((s) => {
    const k = s.customerId ?? "unknown";
    freq[k] = (freq[k] || 0) + 1;
  });
  return Object.entries(freq).map(([customerId, orderCount]) => ({
    customerId,
    orderCount,
  }));
}
...
export async function getDeliveryStatusBreakdown(
  ...
  if (status)
    deliveryWhere.completedAt = status === "completed" ? { not: null } : null;
  ...
  return results.map((r) => ({
    reason: r.failureReason || "Completed",
    count: r._count.id,
  }));
}

export async function getDeliveryTimeAnalysis(
  workspaceId: string,
  start?: Date,
  end?: Date,
  status?: string,
  ...
  const times = shipments
    ...
    .map((s) => ({
      shipmentId: s.id,
      deliveryDays:
        (s.actualDelivery!.getTime() - s.createdAt.getTime()) /
        (1000 * 60 * 60 * 24),
    }));
  return times;
}

export async function getDeliveryFailureAnalysis(
  workspaceId: string,
  start?: Date,
  end?: Date,
  status?: string,
  ...
  const deliveries = await prisma.delivery.findMany({
    ...
    select: {
      failureReason: true,
      shipment: { select: { courierPartnerId: true } },
    },
  });
...
export async function getRevenueByCourier(
  workspaceId: string,
  start?: Date,
  end?: Date,
) {
...
export async function getRevenueByRegion(
  ...
  return Object.entries(grouped).map(([region, revenue]) => ({
    region,
    revenue,
  }));
}
...
export async function getRegionalHeatmap(
  workspaceId: string,
  start?: Date,
  end?: Date,
) {
  ...
  return Object.entries(matrix).map(([from, destinations]) => ({
    from,
    destinations,
  }));
}
...
export async function getComparisonMetrics(
  ...
  const where = dateWhere(workspaceId, start, end);
  if (compareBy === "courier" && entities?.length)
    (where as any).courierPartnerId = { in: entities };
  const shipments = await prisma.shipment.findMany({
    where,
    select: {
      status: true,
      shippingCost: true,
      courierPartnerId: true,
      customerId: true,
    },
  });
  return {
    ...
    failedShipments: shipments.filter((s) => s.status === ShipmentStatus.FAILED)
      .length,
...
export async function getHistoricalComparison(
  ...
  const current = await getComparisonMetrics(
    workspaceId,
    start,
    end,
    compareBy,
